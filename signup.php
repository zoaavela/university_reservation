    ...
    // Vérifier la longueur du mot de passe
        ...
        if (mysqli_num_rows($resultat) > 0) {
            ...
            if ($role == "etudiant") {
                ...
                $promotion = nettoyer_donnees($_POST["promotion"]);
                $tp = nettoyer_donnees($_POST["tp"]);
                $td = nettoyer_donnees($_POST["td"]);
                
                // Insérer l'étudiant dans la base de données
                $sql = "INSERT INTO utilisateur (nom, prenom, email, mot_de_passe, role, numero_etudiant, promotion, tp, td) 
                        VALUES ('$nom', '$prenom', '$email', '$mot_de_passe_hache', '$role', '$numero_etudiant', '$promotion', '$tp', '$td')";
            } else {
                // C'est un enseignant
